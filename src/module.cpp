...
// Module::Module() {}
Module::~Module() { this->valid_ = false; }

void Module::switch_to_cuda() {
...
void Module::to_device(const std::string &new_device) {
...
void Module::add_layer(std::shared_ptr<BaseLayer> layer)
/*
...
void Module::compute_input_output_size()
...
void Module::init_output_state_buffer()
/*
 */
{
    if (this->device.compare("cpu") == 0) {
...
void Module::init_delta_state_buffer()
/*
 */
{
    if (this->device.compare("cpu") == 0) {
...
void Module::set_threads(unsigned int num_threads)
/*
 */
{
...
void Module::preinit_layer(const std::vector<float> &mu_x,
...
void Module::forward(const std::vector<float> &mu_x,
                     const std::vector<float> &var_x)
/*
 */
{
    ...
    for (auto &layer : this->layers) {
        ...
        if (current_layer->get_layer_type() == LayerType::ResNetBlock) {
            ...
            for (int i = 0; i < this->input_z_buffer->size; i++) {
                this->input_z_buffer->mu_a[i] += timesteps[i / num_blocks];
            }
            ...
            // this->input_z_buffer->var_a, batch_size);
        }
...
void Module::forward(BaseHiddenStates &input_states)
/*
 */
{
    // Batch size
...
std::tuple<std::vector<float>, std::vector<float>> Module::smoother()
/*
 */
{
...
std::string Module::get_layer_stack_info() const {
    std::stringstream ss;
    for (const auto &layer : this->layers) {
        if (layer) {
...
void Module::save_csv(const std::string &filename)
/*
...
void Module::load_csv(const std::string &filename)
/*
 */
{
...
std::vector<std::reference_wrapper<std::vector<float>>> Module::parameters()
...
{
    if (!this->valid_) {
        throw std::runtime_error("Module object is no longer valid");
    }
...
void Module::params_from(const Module &model_ref) {
...
// Python Wrapper
void Module::forward_py(pybind11::array_t<float> mu_a_np,
