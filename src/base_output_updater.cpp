 ...
 */
{
    ...
    for (int i = 0; i < B; i++) {
        for (int j = 0; j < no; j++) {
            output_states.var_a[i * no + j] += obs.var_obs[j];
        }
    }

    int end_chunk_mrelu = output_states.actual_size * output_states.block_size;
    ...
    std::vector<float> mu_log =
        std::vector<float>(output_states.actual_size, 0.0f);
 ...
 */
{
    ...
    int B = output_states.block_size;        // Number of batches
    int no = output_states.actual_size / B;  // Number of outputs per batch
