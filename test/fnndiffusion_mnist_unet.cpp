...
LayerBlock create_layer_block(int channels) {
    return LayerBlock(
        ReLU(), LayerNorm(std::vector<int>({channels, IMG_SIZE, IMG_SIZE})),
        Conv2d(channels, channels, 3, false, 1, 1, 1), ReLU(),
        LayerNorm(std::vector<int>({channels, IMG_SIZE, IMG_SIZE})),
        Conv2d(channels, channels, 3, false, 1, 1, 1));
}
   ...
   public:
    UNetDiffusion()
        : Module(
              ...
              ResNetBlock(create_layer_block(N_CHANNELS)), ReLU(),
              Conv2d(N_CHANNELS, N_CHANNELS * 2, 4, false, 1, 1, 1),
              ResNetBlock(create_layer_block(N_CHANNELS * 2)), ReLU(),
              Conv2d(N_CHANNELS * 2, N_CHANNELS * 4, 4, false, 1, 1, 1),
              ResNetBlock(create_layer_block(N_CHANNELS * 4)), ReLU(),
              ConvTranspose2d(N_CHANNELS * 4, N_CHANNELS * 2, 4, true, 1, 1, 1),
              ResNetBlock(create_layer_block(N_CHANNELS * 2)), ReLU(),
              ConvTranspose2d(N_CHANNELS * 2, N_CHANNELS, 4, true, 1, 1, 1),
              ResNetBlock(create_layer_block(N_CHANNELS)), ReLU(),
    ...
    void forward(const std::vector<float>& mu_x,
        ...
        for (auto& layer : this->layers) {
            ...
            if (current_layer->get_layer_type() == LayerType::ResNetBlock) {
                ...
                //                 for (int i = 0; i <
                //                 this->input_z_buffer->size; i++) {
                //                     this->input_z_buffer->mu_a[i] +=
                //                     timesteps[i / num_blocks];
                //                 }
                ...
                //                         dynamic_cast<HiddenStateCuda*>(
                //                             this->input_z_buffer.get());
                //                     cu_input_states->chunks_to_device(
                //                         this->input_z_buffer->size);
                //                 }
                // #endif
            } else {
                current_layer->forward(*this->input_z_buffer,
    ...
    std::vector<std::vector<float>> sample_ddpm(int nsamples, int nfeatures) {
        ...
        for (int t = DIFFUSION_STEPS - 1; t > 0; --t) {
            ...
            for (int i = 0; i < nsamples; i += BATCH_SIZE) {
                ...
                // // Concatenate the noised images with the repeated
                // timesteps std::vector<std::vector<float>>
                // xbatch_timesteps(
                //     batch_size, std::vector<float>(nfeatures + 4));

                // for (size_t j = 0; j < xbatch.size(); ++j) {
                //     // Copy the noised image data
                //     for (size_t k = 0; k < nfeatures; ++k) {
                //         xbatch_timesteps[j][k] = xbatch[j][k];
                //     }

                //     // Repeat the timestep 4 times
                //     for (size_t t = 0; t < 4; ++t) {
                //         xbatch_timesteps[j][nfeatures + t] =
                //         timestep[j][t];
                //     }
                // }
    ...
    void train(const std::vector<std::vector<float>>& X) {
        ...
        for (int epoch = 0; epoch < NEPOCHS; ++epoch) {
            ...
            for (size_t i = 0; i < X.size(); i += BATCH_SIZE) {
                ...
                // noised_timesteps(
                //     Xbatch.size(),
                //     std::vector<float>(IMG_SIZE * IMG_SIZE +
                //                        4));  // Add space for 4 timesteps

                // for (size_t j = 0; j < Xbatch.size(); ++j) {
                //     // Copy the noised image data
                //     for (size_t k = 0; k < IMG_SIZE * IMG_SIZE; ++k) {
                //         noised_timesteps[j][k] = noised[j][k];
                //     }

                //     // Repeat the timestep 4 times
                //     float timestep_value =
                //         static_cast<float>(timesteps[j]) /
                //         DIFFUSION_STEPS;
                //     for (size_t t = 0; t < 4; ++t) {
                //         noised_timesteps[j][IMG_SIZE * IMG_SIZE + t] =
                //             timestep_value;
                //     }
                // }
...
int unet_diffuser() {
    ...
    // images = UNetDiffusion::resize_images(images, 28, 28, IMG_SIZE,
    // IMG_SIZE);
