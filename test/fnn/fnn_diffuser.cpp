...
class MNISTDiffusion : public Module {
   ...
   public:
    MNISTDiffusion()
        : Module(Linear(IMG_SIZE * IMG_SIZE + 4, N_CHANNELS), ReLU(),
    ...
    std::vector<std::vector<float>> sample_ddpm(int nsamples, int nfeatures) {
        ...
        for (int t = DIFFUSION_STEPS - 1; t > 0; --t) {
            ...
            for (int i = 0; i < nsamples; i += BATCH_SIZE) {
                ...
                // Concatenate the noised images with the repeated timesteps
                std::vector<std::vector<float>> xbatch_timesteps(
                ...
                for (size_t j = 0; j < xbatch.size(); ++j) {
                    ...
                    for (size_t t = 0; t < 4; ++t) {
                        xbatch_timesteps[j][nfeatures + t] = timestep[j][t];
                    }
                }

                // Flatten xbatch
                std::vector<float> xbatch_flat;
                for (const auto& row : xbatch_timesteps) {
    ...
    void train(const std::vector<std::vector<float>>& X) {
        ...
        for (int epoch = 0; epoch < NEPOCHS; ++epoch) {
            ...
            for (size_t i = 0; i < X.size(); i += BATCH_SIZE) {
                ...
                // Concatenate the noised images with the repeated timesteps
                std::vector<std::vector<float>> noised_timesteps(
                    Xbatch.size(),
                ...
                for (size_t j = 0; j < Xbatch.size(); ++j) {
                    ...
                    float timestep_value =
                        static_cast<float>(timesteps[j]) / DIFFUSION_STEPS;
                    for (size_t t = 0; t < 4; ++t) {
                ...
                for (const auto& row : noised_timesteps) {
    ...
    void generate_samples(int epoch) {
        ...
        std::string output_file = "./test/fnn/images_fnn/output_image_" +
